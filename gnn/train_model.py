    ...
    def evaluate(self, data, test_batch_size=20, write_info=False):
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            for i in range(len(answer_lists)):
                ...
                # Convert prediction distribution to candidate2prob format
                candidates = data.candidate_entities[data.sample_ids[i]]
                candidate2prob = []
                for j, (candidate, prob) in enumerate(zip(candidates, cur_pred_dist)):
                    if candidate < len(self.entity2id):  # Skip padding entities
                        candidate2prob.append((candidate, prob.item()))
                
                p, r, f1, h1 = self.model.f1_and_hits(cur_ans_list, candidate2prob)
                all_f1.append(f1)
                all_h1.append(h1)
